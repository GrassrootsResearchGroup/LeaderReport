...
if (FALSE){
  n=1000
  test.data.frame = data.frame(Response.ID=1:n) %>% mutate(c1.Q1 = rand_cat(n), c2.Q1 = rand_cat(n),
                                                  c1.Q2 = rand_yn(n), c2.Q2 = rand_yn(n),
                                                  c1.Q3 = rand_cat(n), c2.Q3 = rand_yb(n),
                                                  c3.Q3 = ifelse(c2.Q3 != '', rand_cat(n), ''),
                                                  c1.Q4 = rand_yn(n))
  
  rank_cols = rand_rank(n)
  names(rank_cols) = paste0('c',1:5,'.Q5')
  
  #10-14
  test.data.frame = cbind(test.data.frame, rank_cols)
  #15-17
  test.data.frame = test.data.frame %>% mutate(c1.Q6 = rand_cat(n), c2.Q6 = rand_cat(n), 
                                               c3.Q6 = rand_cat_blank(n))
  
  head(test.data.frame)
  
  #PASS
  q1_expand = expand_column_group(test.data.frame, colrange=2:3, summary_type='')
  
  #PASS (with slight modification)
  q2_expand = expand_column_group(test.data.frame, colrange=4:5, summary_type='yn')
  
  #PASS
  q3_expand = expand_column_group(test.data.frame, colrange=6, other_pair=7:8, summary_type='')
  
  #PASS (NA values should be filtered, but it may be informative for other purposes)
  q5_expand = expand_column_group(test.data.frame, colrange=10:14, summary_type='rank')
  
  #PASS
  q6_expand = expand_column_group(test.data.frame, colrange=15:16, other_pair=17, summary_type='')
}
...
resort_by_frequency <- function(data, cutoff=1, max_fields = 30, mode='yn', 
  ...
  if (mode=='yn'){
    ...
    if (any(is.na(data$variable)))
      data$variable = mapvalues(addNA(data$variable), from=NA, to="Other")
    return(data)
  }
  else if (mode=='ranked'){
